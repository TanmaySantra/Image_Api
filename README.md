...
python
Copy code
gaussian_blur(input_path: str, params: dict, output_path: str)
Description:
...
python
Copy code
edge_detection(input_path: str, output_path: str)
Description:
Detects edges using a Laplacian kernel and outputs a grayscale edge map.

Notes:
This function internally converts the image to grayscale using .convert("L") before applying convolution.

3. Grayscale Conversion
python
Copy code
grayscale_tool(input_path: str, params: dict, output_path: str)
Description:
...
Parameter	Type	Default	Description
method	str	"simple"	"simple" → averages R, G, B equally; "weighted" → uses 0.299R + 0.587G + 0.114B for natural brightness.

4. Binary Threshold
python
Copy code
binary_image(input_path: str, params: dict, output_path: str)
Description:
...
bash
Copy code
curl -X POST "http://127.0.0.1:8000/gaussian-blur" \
  -F "file=@sample.jpg" \
  -F "size=5" \
  -F "sigma=1.2"
